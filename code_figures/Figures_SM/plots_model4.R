...
fn <- "../organized_results/bioII_2008-7_4.RData"
fn_name <- "Biodiversity II (2008) model 4"
...
plot_tree_m4 <- function(resultsfile, scalefac, label){
  load(resultsfile)
  ...
  df_bar_data <- tibble(id = sp_names,
                        bar_value1 = results$pars_orig[(2*n):(3*n-1)],
                        bar_value2 = results$pars_orig[(3*n):(4*n-1)])
  ...
  pl <- ggtree(tree, aes(colour = scalefac * log(branch.length)), 
    ...
    geom_facet(panel = expression(gamma), data = df_bar_data, geom = geom_barh,stat='identity',
               aes(x = bar_value1), width = .6, fill="#F7A90C", color = NA) +
    geom_facet(panel = expression(theta), data = df_bar_data, geom = geom_barh,stat='identity',
               aes(x = bar_value2), width = .6, fill="#6C2691", color = NA) +
    theme(legend.position = "none") + ggtitle(label)
  return(pl)
}


pl_tree <- plot_tree_m4(fn, 3, fn_name)
pl_tree


## Creating all the plots
files <- list.files(path = "../organized_results/", pattern = "\\_4.RData$")
...
for(i in 1:length(fn)){
  pl_tree <- plot_tree_m4(fn[i], 3, paste0(fn_name[i], " model 4"))
  ggsave(filename = paste0("figures_models_2-4/", fn_name[i], "_m4.pdf"), plot = pl_tree)
  print(i)
}
