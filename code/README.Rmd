...
Assume the working directory is the same as the one where the code is stored. 
The input is a csv file containing the biomass of species *j* in plot *i* in cell *i,j*, hence rows represent the different plots and columns represent species in the plots and the values represent the biomass achieved by the species at the end of the growing season. The column names of the file contains the names of the species. For example:
...
In a file with the same name, but ending in "_V.csv" we store the matrix detailing the topology of the tree. Here columns are the species (in the same order as the previous file), rows are ancestors, and a 1 marks the fact that the row/ancestor is a progenitor of the column/species (i.e., whether the species in the column belongs to the clade defined by the ancestor in the row). For example:
...
For each data set, one can run the code using one of four models (see manuscript). Model 1 is the model presented in the main text, whereas models 2-4 are presented in the supplementary material. The syntax is as follows:
